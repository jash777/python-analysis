...
def process_ai_analysis(analysis_id: str, report_text: str):
    ...
    try:
        ...
        # Get AI analysis
        analysis_result = ai_agent.analyze_security_report(report_text)
        
        # Update progress to 60%
        ai_analysis_jobs[analysis_id]['progress'] = 60
        
        # Format the report
        ai_report = ai_agent.format_enhanced_report(analysis_result)
        
        # Save AI report in AI_REPORTS_FOLDER
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
